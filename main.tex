...
\and
\IEEEauthorblockN{2\textsuperscript{nd} Anastasija Sevcenko}
\IEEEauthorblockA{\textit{Cyber-Physical Systems Laboratory} \\
\textit{Institute of Electronics and Computer Science}\\
Riga, Latvia }
\and
...
\begin{abstract}
    Based on the comprehensive evaluation of electrodes for BioImpedance (BioZ) and Body-Coupled Communication (BCC) within biomedical applications, this study investigates the performance metrics—Signal-to-Noise Ratio (SNR), Settling Time, and Chip Error Rate (CER)—of Ag/AgCl, EMS, and gold-plated electrodes. By rigorously testing across scenarios for heart rate monitoring, breathing measurement, and data transmission through the human body, the research offers nuanced insights into electrode suitability. Key findings highlight the contextual performance advantages of each electrode type, laying a foundation for future innovations in non-invasive diagnostics and wearable technology. This contributes significantly to the electrode material selection, emphasizing the balance between application-specific requirements and user comfort
\end{abstract}
